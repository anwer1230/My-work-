...
app.post('/api/auth/restore-session', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const errMsg = e?.message || 'انتهت صلاحية الجلسة السحابية. يرجى تسجيل الدخول مجدداً.';
    console.log('Session restore info (expired or invalid session):', errMsg);
    res.status(200).json({ success: false, expired: true, error: errMsg });
  }
});
...
app.get(['/api/profile/sessions', '/api/sessions'], async (req: Request, res: Response) => {
  try {
...
app.post(['/api/profile/sessions/terminate', '/api/sessions/terminate'], async (req: Request, res: Response) => {
  try {
    const hash = req.body.hash || req.body.id || req.body.session_id;
    if (!hash) {
      return res.status(400).json({ status: 'error', success: false, message: 'Session hash or id is required' });
    }
    if (profileStore.sessions) {
      profileStore.sessions = profileStore.sessions.filter((s: any) => String(s.hash || s.id) !== String(hash));
    }
    broadcastSSE('profile_updated', profileStore);
    res.json({ status: 'ok', success: true, message: 'تم إنهاء الجلسة بنجاح' });
  } catch (e: any) {
    res.status(500).json({ status: 'error', success: false, message: 'تعذر إنهاء الجلسة' });
  }
});

app.post(['/api/profile/sessions/terminate_all', '/api/sessions/terminate_all'], async (req: Request, res: Response) => {
  try {
    if (isTelegramClientActive()) {
    ...
    broadcastSSE('profile_updated', profileStore);
    res.json({ status: 'ok', success: true, message: 'تم إنهاء كافة الجلسات الأخرى بنجاح' });
  } catch (e: any) {
...
⚡ _تم الرصد والتوجيه الفوري بواسطة محرك الأتمتة الذكي_`;
  ...
  broadcastSSE('watchword_alert', {
    ...
    chatId: chat.id,
    targetMsgId: msg.id,
    chatAvatar: chat.avatar || chat.photo,
...
async function executeBulkSend(text: string, targetGroupLinksOrNames?: string[] | string | any, isSmartLoop: boolean = false) {
  ...
  for (const chat of targetChats) {
    ...
    broadcastSSE('log_update', { message: `📤 [${count}/${targetChats.length}] تم الإرسال بنجاح إلى: ${chat.title}` });
...
app.get('/api/saved_links', (req: Request, res: Response) => {
  res.json({ status: 'ok', success: true, links: savedLinksStore });
});
