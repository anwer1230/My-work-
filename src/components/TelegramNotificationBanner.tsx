...
import { X, ExternalLink, BellOff, Volume2, CornerDownLeft, Send } from 'lucide-react';
import { ChatAvatar } from './ChatAvatar';

export interface TelegramNotificationItem {
  id: string;
  chat_id: string | number;
  target_msg_id?: string | number;
  title: string;
  sender_name?: string;
...
interface TelegramNotificationBannerProps {
  ...
  onOpenChat: (chatId: string | number, targetMsgId?: string | number) => void;
  onReply?: (chatId: string | number, text: string) => void;
...
export const TelegramNotificationBanner: React.FC<TelegramNotificationBannerProps> = ({
  ...
  }, [notification, onDismiss, isReplying]);
  ...
  const handleSendReply = (e: React.FormEvent) => {
    ...
    if (replyText.trim()) {
      if (onReply) {
        onReply(notification.chat_id, replyText.trim());
      }
      setVisible(false);
      setTimeout(onDismiss, 300);
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {isReplying ? (
          <form onSubmit={handleSendReply} className="pt-2 border-t border-slate-800/80 flex items-center gap-2">
            <input
              type="text"
              autoFocus
              value={replyText}
              onChange={(e) => setReplyText(e.target.value)}
              placeholder={lang === 'ar' ? 'اكتب ردك السريع هنا...' : 'Type quick reply...'}
              className="flex-1 bg-slate-950/80 border border-slate-700/80 rounded-lg px-2.5 py-1 text-xs text-slate-100 placeholder-slate-500 outline-none focus:border-sky-500 transition"
              onClick={(e) => e.stopPropagation()}
            />
            <button
              type="submit"
              disabled={!replyText.trim()}
              className="px-2.5 py-1 text-xs font-medium rounded-lg bg-sky-500 hover:bg-sky-400 disabled:opacity-50 text-slate-950 transition flex items-center gap-1 shrink-0 shadow-sm"
              onClick={(e) => e.stopPropagation()}
            >
            ...
            <button
              ...
              className="px-2 py-1 text-xs text-slate-400 hover:text-slate-200 rounded-lg hover:bg-slate-800 transition"
            >
          ...
          <div className="flex items-center justify-end gap-2 pt-1 border-t border-slate-800/80">
            ...
            {onReply && (
              <button
                ...
                className="px-2.5 py-1 text-[11px] font-medium rounded-lg bg-slate-800 hover:bg-slate-700 text-sky-400 border border-sky-500/30 transition flex items-center gap-1 shadow-sm"
              >
                <CornerDownLeft className="w-3 h-3" />
                <span>{lang === 'ar' ? 'رد سريع' : 'Reply'}</span>
              </button>
            )}

            <button
              onClick={() => {
                onOpenChat(notification.chat_id, notification.target_msg_id);
                setVisible(false);
                setTimeout(onDismiss, 300);
              }}
