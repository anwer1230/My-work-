...
export const ChatHeader: React.FC<ChatHeaderProps> = ({
  ...
  const isSaved =
    chat.type === 'saved' ||
    String(chat.id) === '1001' ||
    String(chat.id) === 'saved_messages' ||
    chat.username === 'saved' ||
    (chat.title || '').includes('الرسائل المحفوظة') ||
    (chat.title || '').includes('Saved Messages');

  const displayTitle = isSaved ? 'الرسائل المحفوظة' : chat.title;
  const displaySubtitle = isSaved
    ? '📌 مساحتك السحابية لحفظ التنبيهات والرسائل والملفات'
    : chat.type === 'bot'
