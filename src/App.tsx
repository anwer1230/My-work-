...
import { WatchwordsRadarModal } from './components/WatchwordsRadarModal';
import { BulkSenderModal } from './components/BulkSenderModal';
import { AutoJoinerModal } from './components/AutoJoinerModal';
import { RotatingBroadcasterModal } from './components/RotatingBroadcasterModal';
...
import { NertTerminal } from './components/NertTerminal';
...
export function getChatDisplayName(
  chat: { id?: string | number; title?: string; name?: string; type?: string; first_name?: string; last_name?: string; username?: string; is_group?: boolean; is_channel?: boolean } | null | undefined,
  lang: string = 'ar'
): string {
  if (!chat) return '';
  ...
  const titleStr = (chat.title || '').trim();
  const nameStr = (chat.name || '').trim();
  const isSaved =
    chat.type === 'saved' ||
    cidStr === '1001' ||
    cidStr === 'saved_messages' ||
    chat.username === 'saved' ||
    titleStr.includes('الرسائل المحفوظة') ||
    titleStr.includes('Saved Messages') ||
    nameStr.includes('الرسائل المحفوظة') ||
    nameStr.includes('Saved Messages');

  if (isSaved) {
    return lang === 'ar' ? 'الرسائل المحفوظة' : 'Saved Messages';
  }
...
export default function App() {
  ...
  // Standalone Feature Modals (فصل كل واجهة وظيفية لوحدها)
  const [watchwordsRadarModalOpen, setWatchwordsRadarModalOpen] = useState(false);
  const [bulkSenderModalOpen, setBulkSenderModalOpen] = useState(false);
  const [autoJoinerModalOpen, setAutoJoinerModalOpen] = useState(false);
  const [rotatingBroadcasterModalOpen, setRotatingBroadcasterModalOpen] = useState(false);
  ...
  const [nertTerminalOpen, setNertTerminalOpen] = useState(false);
  ...
  // ── SERVICE WORKER INTERACTIVE NOTIFICATION CLICK LISTENER ─────────────────
  useEffect(() => {
    // 1. Handle postMessage from Service Worker when user clicks 'open' or 'reply' on Push Notification
    const handleServiceWorkerMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === 'NOTIFICATION_ACTION_CLICK') {
        const { action, chat_id, data } = event.data;
        const targetChatId = chat_id || data?.chat_id || 1001;
    ...
    // 2. Check URL search params (e.g., /?chat_id=1001&action=open)
    try {
      ...
      if (savedSession) {
        try {
          ...
          if (data.success) {
            ...
            // Session is expired or invalid, clear the stale key
            localStorage.removeItem('tg_session');
          }
        } catch (e) {
          console.log('Saved session restore check failed:', e);
          // If purely offline, use cached data
    ...
    es.onmessage = (event) => {
      try {
        ...
        if (type === 'new_message' || type === 'new_incoming_message') {
          ...
          const targetMsgId = data.targetMsgId || data.alert_data?.msg_id;
          ...
          const chatAvatar = data.chatAvatar || data.chat_avatar;
          const chatType = data.chatType || data.chat_type || 'group';
          ...
          setInAppNotif({
            ...
            chat_type: chatType,
            is_group: chatType === 'group' || chatType === 'supergroup',
            is_channel: chatType === 'channel',
            date: Math.floor(Date.now() / 1000),
          });
          ...
          showPushNotification(`🚨 رصد كلمة مراقبة: ${word}`, {
            ...
            chat_id: targetCid,
            icon: chatAvatar || 'https://telegram.org/img/t_logo.png',
    ...
    <div className="tg-app">
      ...
      <div className={`drawer ${drawerOpen ? 'open' : ''}`} id="drawer">
        ...
        <div className="drawer-items">
            ...
            {automationDropdownOpen && (
              <div style={{ padding: '4px 6px', display: 'flex', flexDirection: 'column', gap: 2 }}>
                <div
                  ...
                  onClick={() => {
                    setDrawerOpen(false);
                    setWatchwordsRadarModalOpen(true);
                  }}
                >
                  <i className="fas fa-satellite-dish" />
                  <span>{lang === 'ar' ? 'رادار المراقبة التلقائية الحية' : 'Live Watchwords Radar'}</span>
                  <span className="compact-badge" style={{ background: 'rgba(245, 158, 11, 0.2)', color: '#f59e0b' }}>
                    {lang === 'ar' ? 'مستقل 🎯' : 'Radar'}
                  </span>
                </div>
                ...
                <div
                  ...
                  onClick={() => {
                    setDrawerOpen(false);
                    setBulkSenderModalOpen(true);
                  }}
                >
                  <i className="fas fa-paper-plane" />
                  <span>{lang === 'ar' ? 'الإرسال التلقائي والحملات' : 'Bulk Sender'}</span>
                  <span className="compact-badge" style={{ background: 'rgba(16, 185, 129, 0.2)', color: '#10b981' }}>
                    {lang === 'ar' ? 'مستقل 📨' : 'Sender'}
                  </span>
                </div>
                ...
                >
                  ...
                  <span className="compact-badge" style={{ background: 'rgba(255, 82, 82, 0.2)', color: '#ff5252' }}>
                    {lang === 'ar' ? 'مستقل 🤖' : 'AutoReply'}
                  </span>
                </div>
                ...
                <div
                  ...
                  onClick={() => {
                    setDrawerOpen(false);
                    setRotatingBroadcasterModalOpen(true);
                  }}
                >
                  <i className="fas fa-sync-alt" />
                  <span>{lang === 'ar' ? 'النشر المتسلسل والدوار' : 'Rotating Send'}</span>
                  <span className="compact-badge" style={{ background: 'rgba(124, 77, 255, 0.2)', color: '#7c4dff' }}>
                    {lang === 'ar' ? 'مستقل 🔄' : 'Rotating'}
                  </span>
                </div>
                ...
                <div
                  ...
                  onClick={() => {
                    setDrawerOpen(false);
                    openAutomationSuite('batches');
                  }}
                >
                  <i className="fas fa-envelope-open-text" />
                  <span>{lang === 'ar' ? 'رسائلي وسجل الدفعات' : 'My Messages'}</span>
                </div>
          ...
          >
            <i className="fas fa-laptop" style={{ color: '#06b6d4' }} />
            <span>{lang === 'ar' ? '📱 الأجهزة والجلسات النشطة (Active Sessions)' : '📱 Active Sessions & Devices'}</span>
          </div>

          {/* Nert Ops & Monitoring Terminal */}
          <div
            className="drawer-item"
            onClick={() => {
              setDrawerOpen(false);
              setNertTerminalOpen(true);
            }}
          >
            <i className="fas fa-terminal" style={{ color: '#10b981' }} />
            <span>{lang === 'ar' ? '💻 طرفية العمليات ونيرت تيرمينال (Nert Terminal)' : '💻 Nert Ops & Monitoring Terminal'}</span>
          </div>
      ...
      <div className={`left-col ${currentChatId && window.innerWidth <= 768 ? 'hidden' : ''}`} id="leftCol">
        ...
        <div className="left-hdr">
          ...
          {/* 🎯 زر المراقبة التلقائية المستقل (رادار الكلمات المفتاحية) */}
          <button
            className="menu-btn"
            style={{ color: '#f59e0b', marginLeft: 2, marginRight: 2 }}
            onClick={() => setWatchwordsRadarModalOpen(true)}
            title={lang === 'ar' ? 'رادار المراقبة التلقائية الفورية' : 'Live Watchwords Radar'}
          >
            <i className="fas fa-satellite-dish" />
          </button>
          {/* 📨 زر الإرسال التلقائي المستقل (الحملات والجدولة) */}
          <button
            className="menu-btn"
            style={{ color: '#10b981', marginLeft: 2, marginRight: 2 }}
            onClick={() => setBulkSenderModalOpen(true)}
            title={lang === 'ar' ? 'الإرسال التلقائي والحملات الجماعية' : 'Bulk Sender & Scheduler'}
          >
            <i className="fas fa-paper-plane" />
          </button>
          {/* 🔍 زر فحص واستخراج الروابط */}
          <button
            className="menu-btn"
            style={{ color: '#00e5ff', marginLeft: 2, marginRight: 2 }}
            onClick={() => openAutomationSuite('link_scraper')}
            title={lang === 'ar' ? 'فحص وفرز الروابط واستخراجها' : 'Link Search & Classifier'}
          >
            <i className="fas fa-search-dollar" />
          </button>
          {/* ⚡ زر منظومة الأتمتة الشاملة */}
          <button
            className="menu-btn"
            style={{ color: '#8b5cf6', marginLeft: 2, marginRight: 2 }}
            onClick={() => openAutomationSuite('batches')}
            title={lang === 'ar' ? 'منظومة إنجاز الكاملة للأتمتة' : 'Enjaz Automation Suite'}
          >
            <i className="fas fa-bolt" />
          </button>
          {/* 📲 زر تثبيت تطبيق Telegram APK المباشر */}
          <button
            className="menu-btn"
        ...
        {/* Quick Tools & Automation Bar (شريط الوصول المباشر للوظائف المستقلة) */}
        <div
          style={{
            display: 'flex',
            alignItems: 'center',
        ...
        >
          <button
            ...
            title={lang === 'ar' ? 'فتح واجهة رادار المراقبة التلقائية الحية' : 'Open Live Watchwords Radar'}
          >
            <i className="fas fa-satellite-dish" />
            <span>{lang === 'ar' ? 'رادار المراقبة 🎯' : 'Radar Monitor'}</span>
          </button>
          ...
          <button
            ...
            title={lang === 'ar' ? 'فتح واجهة الإرسال التلقائي والحملات' : 'Open Bulk Sender'}
          >
            <i className="fas fa-paper-plane" />
            <span>{lang === 'ar' ? 'الإرسال التلقائي 📨' : 'Bulk Sender'}</span>
          </button>
          ...
          <button
            ...
            title={lang === 'ar' ? 'فحص وفرز واستخراج الروابط' : 'Link Search & Classifier'}
          >
            <i className="fas fa-search-dollar" />
            <span>{lang === 'ar' ? 'فرز الروابط 🔍' : 'Link Scraper'}</span>
          </button>

          <button
            onClick={() => setAutoJoinerModalOpen(true)}
            style={{
              display: 'flex',
              alignItems: 'center',
              gap: 6,
            ...
            title={lang === 'ar' ? 'الانضمام التلقائي للمجموعات والقنوات' : 'Auto Joiner'}
          >
            <i className="fas fa-bolt" />
            <span>{lang === 'ar' ? 'انضمام تلقائي ⚡' : 'Auto Join'}</span>
          </button>

          <button
            onClick={() => setRotatingBroadcasterModalOpen(true)}
            style={{
              display: 'flex',
              alignItems: 'center',
              gap: 6,
            ...
            title={lang === 'ar' ? 'النشر المتسلسل والدوار' : 'Rotating Broadcaster'}
          >
            <i className="fas fa-sync-alt" />
            <span>{lang === 'ar' ? 'نشر تناوبي 🔄' : 'Rotating Send'}</span>
          </button>

          <button
            onClick={() => setAutoReplyModalOpen(true)}
            style={{
              display: 'flex',
              alignItems: 'center',
              gap: 6,
            ...
            title={lang === 'ar' ? 'الردود التلقائية الذكية' : 'Auto Reply Rules'}
          >
            <i className="fas fa-robot" />
            <span>{lang === 'ar' ? 'رد تلقائي 🤖' : 'Auto Reply'}</span>
          </button>
        </div>
      ...
      {/* ══ FORWARD MODAL ══ */}
      {fwdModalOpen && (() => {
        const sourceMsg = currentChatId ? (messages[currentChatId] || []).find((m) => m.id === fwdMsgId) : null;
        return (
          <div className="modal-overlay show" onClick={() => setFwdModalOpen(false)}>
            <div className="fwd-modal" onClick={(e) => e.stopPropagation()}>
              ...
              {/* Message / Media Preview Summary */}
              {sourceMsg && (
                <div className="fwd-preview-box">
                  <div className="fwd-preview-header">
                    <i className="fas fa-share" />
                    <span>{lang === 'ar' ? 'معاينة الرسالة الموجهة' : 'Forward Preview'}</span>
                    {sourceMsg.sender_name && (
                      <span className="fwd-preview-sender">
                        • {sourceMsg.sender_name}
                      </span>
                    )}
                  </div>
                  <div className="fwd-preview-content">
                    {sourceMsg.media && (
                      sourceMsg.type === 'photo' || sourceMsg.media.startsWith('data:image') || sourceMsg.media.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                        <img src={sourceMsg.media} alt="" className="fwd-preview-media-thumb" />
                      ) : (
                        <span className="fwd-preview-media-badge">
                          <i className={
                            sourceMsg.type === 'voice' ? 'fas fa-microphone' :
                            sourceMsg.type === 'document' ? 'fas fa-file-alt' :
                            sourceMsg.type === 'audio' ? 'fas fa-music' :
                            'fas fa-paperclip'
                          } />
                          {sourceMsg.type === 'voice' ? (lang === 'ar' ? 'رسالة صوتية' : 'Voice message') :
                           sourceMsg.type === 'document' ? (lang === 'ar' ? 'مستند' : 'Document') :
                           sourceMsg.type === 'audio' ? (lang === 'ar' ? 'ملف صوتي' : 'Audio file') :
                           (lang === 'ar' ? 'ملف مرفق' : 'Attachment')}
                        </span>
                      )
                    )}
                    {sourceMsg.text ? (
                      <span className="fwd-preview-text" title={sourceMsg.text}>
                        {sourceMsg.text}
                      </span>
                    ) : !sourceMsg.media ? (
                      <span className="fwd-preview-text" style={{ fontStyle: 'italic', opacity: 0.7 }}>
                        {lang === 'ar' ? 'رسالة بدون نص' : 'Empty message'}
                      </span>
                    ) : null}
                  </div>
                </div>
      ...
      {/* ══ STANDALONE MODALS (فصل كل واجهة وظيفية لوحدها) ══ */}
      <WatchwordsRadarModal
        isOpen={watchwordsRadarModalOpen}
        onClose={() => setWatchwordsRadarModalOpen(false)}
      />

      <BulkSenderModal
        isOpen={bulkSenderModalOpen}
        onClose={() => setBulkSenderModalOpen(false)}
        chats={chats}
      />

      <AutoJoinerModal
        isOpen={autoJoinerModalOpen}
        onClose={() => setAutoJoinerModalOpen(false)}
      />

      <RotatingBroadcasterModal
        isOpen={rotatingBroadcasterModalOpen}
        onClose={() => setRotatingBroadcasterModalOpen(false)}
        chats={chats}
      />
      ...
      <TelegramNotificationBanner
        ...
        onOpenChat={(cid, targetMsgId) => {
          selectChat(cid, targetMsgId);
          setInAppNotif(null);
        }}
        onReply={async (cid, replyText) => {
          setInAppNotif(null);
          const now = Math.floor(Date.now() / 1000);
          const tmpId = `tmp_${Date.now()}`;
          const replyMsgObj: MessageItem = {
            id: tmpId,
            chat_id: cid,
            text: replyText,
            type: 'text',
          ...
          setMessages((prev) => ({
            ...prev,
            [String(cid)]: [...(prev[String(cid)] || []), replyMsgObj],
          }));

          setChats((prev) =>
            prev.map((c) =>
              String(c.id) === String(cid)
                ? { ...c, lastMsg: replyText, lastMsgDate: now, isOut: true }
                : c
            )
          );

          showToast(lang === 'ar' ? '🚀 تم إرسال الرد السريع بنجاح!' : 'Quick reply sent!');
        ...
        onMuteChat={(cid) => {
          setChats((prev) =>
            prev.map((c) =>
              String(c.id) === String(cid) ? { ...c, muted: true } : c
            )
          );
          showToast(lang === 'ar' ? '🔕 تم كتم تنبيهات المحادثة' : 'Chat muted');
        }}
      ...
      {/* ══ ACTIVE SESSIONS & CONNECTED DEVICES MODAL ══ */}
      <ActiveSessionsModal
      ...
      {/* ══ NERT TERMINAL & OPERATIONS DASHBOARD MODAL ══ */}
      {nertTerminalOpen && (
        <div className="fixed inset-0 z-[2700] bg-zinc-950 flex flex-col overflow-hidden select-none">
          <div className="flex items-center justify-between px-4 py-2 bg-zinc-900 border-b border-zinc-800 shrink-0">
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full bg-emerald-500 animate-ping" />
              <span className="font-bold text-sm text-zinc-200">نيرت تيرمينال | Nert Operations Console</span>
            </div>
            <button
              onClick={() => setNertTerminalOpen(false)}
              className="px-3 py-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-lg text-xs font-bold transition-all"
            >
              ✕ إغلاق الطرفية
            </button>
          </div>
