...
import { AccountInstance } from './lib/accountInstance';
import { MessagesController } from './lib/messagesController';
import { MessagesStorage } from './lib/messagesStorage';
...
export function renderFormattedMessageText(text: string) {
  if (!text) return null;
...
export default function App() {
  ...
  const [hasMoreOlderMessages, setHasMoreOlderMessages] = useState<Record<string | number, boolean>>({});
  ...
  const topMessageSentinelRef = useRef<HTMLDivElement | null>(null);

  // ── INTERSECTION OBSERVER FOR LAZY-LOADING OLDER MESSAGES (SCROLL UP) ──────
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (firstEntry && firstEntry.isIntersecting && !loadingMessages && !loadingMoreMessages) {
          const hasMore = hasMoreOlderMessages[currentChatId] !== false;
          if (hasMore) {
  ...
  }, [currentChatId, messages, loadingMessages, loadingMoreMessages, hasMoreOlderMessages]);
  ...
  const [automationActiveTab, setAutomationActiveTab] = useState<AutomationTab>('send_monitor');
  ...
  const openAutomationSuite = (tab: AutomationTab = 'send_monitor') => {
  ...
  // ── NETWORK STATUS & BACKGROUND SYNC (DrKLO/Telegram Architecture) ─────────
  useEffect(() => {
    backgroundSyncService.init({
      onChatsUpdated: (newChats) => {
        setChats(newChats);
      },
      onMessagesUpdated: (cid, newMsgs) => {
        setMessages((prev) => ({
          ...prev,
          [cid]: newMsgs,
        }));
      },
      onMessageStatusUpdated: (tempId, serverMsg) => {
        const serverId = serverMsg?.id || serverMsg?.message?.id;
        const cid = serverMsg?.chat_id || currentChatId;
        if (cid) {
          setMessages((prev) => {
            const list = prev[cid] || [];
            return {
      ...
      onSyncStatus: (status, stats) => {
        if (status === 'syncing') {
          // syncing
        } else if (status === 'idle' && stats && stats.pendingFlushed > 0) {
          showToast(
            lang === 'ar'
              ? `⚡ تم إرسال ${stats.pendingFlushed} رسالة كانت معلقة في وضع عدم الاتصال!`
              : `⚡ Flushed ${stats.pendingFlushed} offline queued messages!`
          );
        }
      },
    });
  ...
  useEffect(() => {
    backgroundSyncService.setActiveChat(currentChatId);
  }, [currentChatId]);
  ...
  useEffect(() => {
    const handleOnline = () => {
      ...
      if (isLoggedIn) {
        backgroundSyncService.performFullSync({
          onChatsUpdated: (newChats) => setChats(newChats),
          onMessagesUpdated: (cid, newMsgs) => setMessages((prev) => ({ ...prev, [cid]: newMsgs })),
        });
      }
    };
    ...
    const handleOffline = () => {
      setIsOnline(false);
      showToast(lang === 'ar' ? '📡 انقطع الاتصال - يتم حفظ الرسائل للإرسال التلقائي عند عودة الشبكة (Offline Queue)' : '📡 Offline mode - Outgoing messages will sync automatically when back online');
    };
    ...
    async function checkAuth() {
      ...
      if (savedSession) {
        try {
          ...
          if (data.success) {
            ...
            UserConfig.getInstance(0).setCurrentUser(data.user, true);
            if (data.dialogs && data.dialogs.length > 0) {
              setChats(data.dialogs);
              saveCachedChats(data.dialogs);
              MessagesController.getInstance(0).onAuthSuccess({ user: data.user, session: savedSession, dialogs: data.dialogs });
            } else {
              loadChats();
            }
      ...
      try {
        ...
        if (d.success && d.authenticated) {
          ...
          UserConfig.getInstance(0).setCurrentUser(d.user, true);
          loadChats();
          fetchActualProfilePhoto();
        } else {
  ...
  const handleVerifyCode = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        MessagesController.getInstance(0).onAuthSuccess({
          user: data.user,
  ...
  // ── LAZY LOAD OLDER MESSAGES (INTERSECTION OBSERVER) ──────────────────────
  const loadOlderMessages = async (chatId: string | number) => {
    if (loadingMoreMessages || !chatId) return;
    const currentMsgs = messages[chatId] || [];
    if (currentMsgs.length === 0) return;

    const oldestMsg = currentMsgs[0];
    const rawOffsetId = oldestMsg ? String(oldestMsg.id).replace('m_tg_', '').replace('sys_', '').replace('msg_', '') : '0';
    const offsetId = parseInt(rawOffsetId, 10) || 0;

    setLoadingMoreMessages(true);
    try {
      const scrollEl = msgsAreaRef.current;
      const previousScrollHeight = scrollEl ? scrollEl.scrollHeight : 0;
      const previousScrollTop = scrollEl ? scrollEl.scrollTop : 0;

      // MTProto getHistory prioritizing up to 200 items per batch
      const r = await fetch(`/api/chats/${chatId}/messages?limit=200&offset_id=${offsetId}`);
      const d = await r.json();

      if (d.success && d.messages && d.messages.length > 0) {
        const fetchedOlder: MessageItem[] = d.messages.map((m: any) => ({
          ...m,
          type: m.type || (m.media ? (m.media.includes('blob:') || m.media.includes('.mp3') ? 'voice' : 'photo') : 'text'),
        }));

        // Filter out any duplicates
        const existingIds = new Set(currentMsgs.map((m) => String(m.id)));
        const newBatch = fetchedOlder.filter((m) => !existingIds.has(String(m.id)));

        if (newBatch.length > 0) {
          const combined = [...newBatch, ...currentMsgs].sort((a: any, b: any) => (a.date || 0) - (b.date || 0));
          setMessages((prev) => ({
            ...prev,
            [chatId]: combined,
          }));
          saveCachedMessages(chatId, combined);

          // Preserve exact scroll position after prepending older messages
      ...
      console.warn('Failed to lazy load older messages via MTProto:', e);
    }
  ...
  const selectChat = async (id: string | number) => {
    ...
    if (cachedMsgs && cachedMsgs.length > 0) {
      setMessages((prev) => ({
        ...prev,
        [id]: cachedMsgs.slice(-200), // Prioritize last 200 messages initially
      }));
    }
    ...
    // Fetch prioritized last 200 messages via Telegram MTProto RPC
    setLoadingMessages(true);
    try {
      const r = await fetch(`/api/chats/${id}/messages?limit=200`);
      const d = await r.json();
      if (d.success && d.messages) {
        ...
        setHasMoreOlderMessages((prev) => ({
          ...prev,
          [id]: !!d.has_more || fetchedMsgs.length >= 200,
        }));
    ...
    <div className="tg-app">
      ...
      <div className={`drawer ${drawerOpen ? 'open' : ''}`} id="drawer">
        ...
        <div className="drawer-items">
          ...
          >
            <i className="fas fa-shield-alt" style={{ color: '#00e5ff' }} />
            <span>{lang === 'ar' ? 'الخصوصية والأمان' : 'Privacy & Security'}</span>
          </div>
            ...
            {automationDropdownOpen && (
              <div style={{ padding: '4px 6px', display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                <div
                  className="drawer-item-compact"
                  style={{ color: '#10b981' }}
                  onClick={() => openAutomationSuite('accounts')}
                >
                  <i className="fas fa-users-cog" />
                  <span>{lang === 'ar' ? 'إدارة الحسابات' : 'Accounts Manager'}</span>
                </div>
              </div>
            )}
          </div>
      ...
      <div className={`left-col ${currentChatId && window.innerWidth <= 768 ? 'hidden' : ''}`} id="leftCol">
        ...
        <BottomNavBar
          ...
          onSelectTab={(tab) => {
            ...
            else if (tab === 'automation') openAutomationSuite('send_monitor');
        ...
        {!currentChatId ? (
          ...
          <>
              ...
              return (
                <div className="chat-hdr">
                  ...
                  <div className="hdr-actions">
                    {/* Voice Call Button - Visible on private chats or desktop */}
                    ...
                    {/* Chat Info (Desktop only) */}
                    <button
                      className="icon-btn hidden md:flex"
                      onClick={() => currentChat && openProfile(currentChat)}
                      title={lang === 'ar' ? 'معلومات المحادثة' : 'Chat Info'}
                    >
                    ...
                    {/* Chat Header Dropdown Menu (DrKLO/Telegram Hierarchy) */}
                    {chatHdrMenuOpen && (
                      <div
                      ...
                      >
                        ...
                        >
                          <i className="fas fa-info-circle" style={{ color: '#2481cc' }} />
                          <span>{lang === 'ar' ? 'معلومات المحادثة' : 'Chat Info'}</span>
                        </div>
                        ...
                        >
                          ...
                          <span>{lang === 'ar' ? 'مكالمة فيديو' : 'Video Call'}</span>
                        </div>
                        ...
                        >
                          <i className="fas fa-poll" style={{ color: '#f59e0b' }} />
                          <span>{lang === 'ar' ? 'إنشاء استطلاع رأي متقدم' : 'Create Poll'}</span>
                        </div>
            ...
            <div className="msgs-area custom-scrollbar" ref={msgsAreaRef} id="msgsArea">
              ...
              <div ref={topMessageSentinelRef} style={{ height: 1, width: '100%', pointerEvents: 'none' }} />

              {/* Lazy Loading Spinner for Older Messages */}
              {loadingMoreMessages && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8px 0', gap: 6 }}>
                  <div className="list-loader" style={{ margin: 0, transform: 'scale(0.8)' }}>
                    <div className="dot" />
                    <div className="dot" />
                    <div className="dot" />
                  </div>
                  <span style={{ fontSize: 11, color: 'var(--text2)', fontWeight: 600 }}>
                    {lang === 'ar' ? 'جاري تحميل الرسائل الأقدم...' : 'Loading older messages...'}
                  </span>
                </div>
              )}
              ...
              {loadingMessages ? (
                ...
                currentChatMsgs.map((m) => {
                  const isOut = !!m.out || !!m.from_me;

                  // Render System Messages (bans, restrictions, joins, admin changes, pin, title change)
                  if (m.is_system || m.type === 'system' || m.sender_id === 'system') {
                    return (
                      <SystemMessageItem
                        key={m.id}
                        text={m.text || 'إشعار نظام'}
                        type={m.system_type as any}
                        date={m.date}
                        isMe={isOut}
                      />
                    );
                  ...
                  return (
                    ...
                    >
                      <div className={`bubble ${isOut ? 'out' : 'in'}`}>
                        ...
                        {/* Forward Info */}
                        {m.fwd_from && (
                          <div style={{ fontSize: 11, color: 'var(--tg-blue)', marginBottom: 4 }}>
                            <i className="fas fa-share" /> {lang === 'ar' ? `محولة من ${m.fwd_from}` : `Forwarded from ${m.fwd_from}`}
                          </div>
                        )}

                        {/* Sender Name in groups */}
                        {!isOut && m.sender_name && currentChat?.type === 'group' && (
                          <div style={{ fontSize: 12, fontWeight: 700, color: avatarColor(m.sender_id), marginBottom: 4 }}>
                            {m.sender_name}
                          </div>
                        )}
                        ...
                        {/* Photo Media */}
                        {isPhoto && m.media && (
                          <div style={{ borderRadius: 8, overflow: 'hidden', marginBottom: 6, cursor: 'pointer' }} onClick={() => openLightboxModal(m.media || '')}>
                            <img src={m.media} alt="" style={{ maxWidth: '100%', maxHeight: 320, objectFit: 'cover', display: 'block', borderRadius: 8 }} />
                          </div>
                        )}
            ...
            {/* Input Bar or Read-Only Restricted Chat Bar */}
            {currentChat && (currentChat.type === 'channel' && !currentChat.restrictions?.is_admin && !currentChat.restrictions?.is_creator) ? (
              <div className="channel-readonly-bar">
                <i className="fas fa-bullhorn" style={{ color: '#2481cc', fontSize: 16 }} />
                <span>{lang === 'ar' ? 'القناة مخصصة للبث فقط (المشرفون فقط يمكنهم النشر)' : 'Channel is in read-only broadcast mode (Admins only)'}</span>
                {(currentChat.muted || currentChat.is_muted) ? (
                  <button
                    className="unmute-channel-btn"
                    onClick={() => {
                      if (currentChatId) {
                        setChats((prev) =>
                          prev.map((c) =>
                            String(c.id) === String(currentChatId) ? { ...c, muted: false, is_muted: false } : c
                          )
                        );
                        showToast(lang === 'ar' ? 'تم تفعيل التنبيهات' : 'Unmuted');
                      }
                    }}
                  >
                    <i className="fas fa-volume-up" /> {lang === 'ar' ? 'إلغاء الكتم' : 'Unmute'}
                  </button>
                ) : (
                  <button
                    className="mute-channel-btn"
                    onClick={() => {
                      if (currentChatId) {
                        setChats((prev) =>
                          prev.map((c) =>
                            String(c.id) === String(currentChatId) ? { ...c, muted: true, is_muted: true } : c
                          )
                        );
                        showToast(lang === 'ar' ? 'تم كتم التنبيهات' : 'Muted');
                      }
                    }}
                  >
                    <i className="fas fa-volume-mute" /> {lang === 'ar' ? 'كتم الإشعارات' : 'Mute'}
                  </button>
                )}
              </div>
            ) : currentChat && (currentChat.banned_rights?.send_messages || currentChat.restrictions?.banned_rights?.send_messages) ? (
              <div className="banned-readonly-bar">
                <i className="fas fa-ban" style={{ color: '#ef4444', fontSize: 16 }} />
                <span>{lang === 'ar' ? 'تم تقييدك من قبل المشرفين: لا يمكنك إرسال رسائل في هذه المجموعة' : 'You are restricted by admins from posting messages'}</span>
              </div>
            ) : (
      ...
      {/* ══ PRIVACY & SECURITY MODAL (Telegram 12.x / DrKLO Architecture) ══ */}
