...
export async function fetchDialogsSafe(client: TelegramClient): Promise<any[]> {
  try {
    ...
    return dialogs.map((d: any) => {
      ...
      const isSelf = !!(d.isUser && (d.entity?.isSelf || d.entity?.self || d.name === 'me' || d.title === 'me'));
      const type = isSelf ? 'saved' : d.isGroup ? 'group' : (isBroadcast || d.isChannel) ? 'channel' : d.isUser ? 'private' : (d.entity?.title ? 'group' : 'private');
      ...
      const username = isSelf ? 'saved' : (d.entity?.username ? `@${d.entity.username}` : undefined);
...
export async function restoreTelegramSession(sessionString: string): Promise<{ status: string; user?: any; dialogs?: any[] }> {
  ...
  try {
    ...
    try {
      ...
      console.warn('Fallback connect failed:', e);
    }
  }
